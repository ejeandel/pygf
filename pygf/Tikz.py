...
class TikzLayer(Layer):
    ...
    def text(self, point, text, **hints):
        ...
        self.nodelayer += "\\node[{opts}] at ({position}) {{{text}}};\n".format(
            opts=dic_to_list(opts),
            text=escape(text),
            position=self.transform(point))
    ...
    def new_brace(self, end, path, msg):
        ...
        start = path[0][0]
        self.edgelayer += r"\draw [decorate,decoration={brace,amplitude=10pt},xshift=-4pt] (%d.center) -- (%d.center) node [black,midway,xshift=-1cm] {%s};" % (
            self.nodes[end], self.nodes[start], msg) + "\n"
